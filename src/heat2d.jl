...
is_stable(model) = is_stable(model.params)
max_stable_dt(model) = max_stable_dt(model.params)
...
function Heat2D(field::Field2D{T};
    ...
    if check_stability && !is_stable(params)
        throw(ArgumentError("""
            Unstable configuration: CFL number is $(cfl_number(params)), which exceeds the
            explicit-scheme limit of 0.5. The run would diverge to NaN.

            Fix it by reducing dt to at most $(max_stable_dt(params)), reducing alpha, or
            coarsening the grid. To explore the instability on purpose, construct the
